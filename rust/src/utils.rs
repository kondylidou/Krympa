...
pub fn precompute_lemmas(
    ...
    for entry in fs::read_dir(proofs_dir).map_err(|e| e.to_string())? {
        ...
        // canonical lemma name (remove prover suffixes)
        let lemma_name = path
            .file_stem()
            .and_then(|s| s.to_str())
...
pub fn select_actual_lemma(proofs_dir: &str, lemma_name: &str) -> Option<String> {
    ...
    let suffixes = ["_twee.proof", "_vampire.proof", "_iprover.proof"];
...
/// Strips the prover suffix (_twee, _vampire, _iprover) from a lemma name if present
pub fn strip_prover_suffix(lemma_name: &str) -> String {
    let suffixes = ["_twee", "_vampire", "_iprover"];
    for suf in &suffixes {
